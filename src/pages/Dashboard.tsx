...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (billingStatus?.currentMonth) setSelectedBillingPeriod(billingStatus.currentMonth);
      ...
      <div className="space-y-6" data-testid="dashboard-content">
        ...
        {!setupComplete ? (
          ...
          <>
            ...
            <Modal isOpen={showGenerateModal} onClose={() => setShowGenerateModal(false)} title="Generate Bills">
              <div className="p-6">
                <p className="text-sm text-slate-700 dark:text-slate-300 mb-3">
                  Generate maintenance bills for the selected period. This will create charges for all flats in the society.
                </p>
                <label className="text-xs font-medium text-slate-500">Billing Period</label>
                <input
                  type="month"
                ...
                />
                <p className="text-xs text-slate-500">Default: {billingStatus?.currentMonth ?? 'current month'}</p>
              </div>
              <ModalFooter>
