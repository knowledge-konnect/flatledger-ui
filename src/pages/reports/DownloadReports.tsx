...
export default function DownloadReportsPage() {
  ...
  useEffect(() => {
    const safeMonth = clampMonth(monthValue);
    if (safeMonth !== monthValue) {
      setMonthValue(safeMonth);
    }
  }, [monthValue, clampMonth]);
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      value={monthValue}
                      min={minMonth}
                      max={maxMonth || MAX_MONTH_VALUE}
                      aria-label="Select month"
                      onChange={e => setMonthValue(clampMonth(e.target.value))}
                      className={cn(
